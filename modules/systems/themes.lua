...
function customiser:get()
	local array = {}
    if isfolder("Solar Hub\\Themes") then
		for i, v in next, listfiles("Solar Hub\\Themes") do
...
function customiser:load(name)
	local path = "Solar Hub/Themes/" .. name .. ".json"
	if isfile(path) then
...
function customiser:save(theme, name)
	...
	if succ then
		writefile("Solar Hub/Themes/" .. name .. ".json", json)
	end
end
